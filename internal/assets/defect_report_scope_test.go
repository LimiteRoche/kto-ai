...
// k.to provider or tool defect", and from the user's seat everything that
...
// be doing when it happened. A k.to workflow merely HOSTING a failure is
...
func TestDefectHandoffAdmitsOnlyFailuresGentleAIProduced(t *testing.T) {
	for _, path := range allSDDOrchestratorAssetPaths(t) {
		t.Run(path, func(t *testing.T) {
			...
			if strings.Contains(content, "appears blocked by a k.to provider or tool defect") {
