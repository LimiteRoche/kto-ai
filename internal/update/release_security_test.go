	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			command.Env = append(environment,
				...
				"GITHUB_REPOSITORY=LimiteRoche/kto-ai",
...
func TestGoReleaserSignsBoundManifestAndInjectsTrustAnchors(t *testing.T) {
	...
	for _, required := range []string{
		...
		`github.com/LimiteRoche/kto-ai/v2/internal/update/upgrade.releaseMinisignPublicKeys={{ .Env.MINISIGN_PUBLIC_KEYS_CANONICAL }}`,
		"-trimpath",
	} {
...
func TestCanonicalReleasePublicKeysControlRealLinkerBuild(t *testing.T) {
	...
	const linkerTarget = "github.com/LimiteRoche/kto-ai/v2/internal/update/upgrade.releaseMinisignPublicKeys"
