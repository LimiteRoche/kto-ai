...
func GentleAISourceInstallCommand(version string) string {
	...
	return "go install github.com/LimiteRoche/kto-ai/v2/cmd/gentle-ai@" + target
}
...
func gentleAIHint(profile system.PlatformProfile) string {
	...
	switch profile.OS {
	case "linux":
		return "curl -fsSL https://raw.githubusercontent.com/LimiteRoche/kto-ai/main/scripts/install.sh | bash"
	case "darwin":
