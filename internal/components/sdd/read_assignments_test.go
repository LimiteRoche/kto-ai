...
func TestReadCurrentModelAssignments(t *testing.T) {
	...
	content := `{
  "agent": {
    "kto-orchestrator": { "model": "anthropic:claude-sonnet-4-20250514" },
	...
	tests := []struct {
		...
		{"kto-orchestrator", "anthropic", "claude-sonnet-4-20250514"},
...
func TestReadCurrentModelAssignmentsMapsLegacyOrchestratorToGentleOrchestrator(t *testing.T) {
  ...
  "agent": {
	...
	if got["kto-orchestrator"] != want {
		t.Fatalf("kto-orchestrator assignment = %+v, want %+v", got["kto-orchestrator"], want)
	}
}
...
func TestReadCurrentModelAssignmentsPartialModels(t *testing.T) {
	...
	content := `{
  "agent": {
    "kto-orchestrator": { "model": "anthropic:claude-opus-4-5" },
	...
	// Only kto-orchestrator has a model — only it should appear
	if len(got) != 1 {
		t.Errorf("ReadCurrentModelAssignments() len = %d, want 1; got %v", len(got), got)
	}

	a, ok := got["kto-orchestrator"]
	if !ok {
		t.Fatal("kto-orchestrator missing from result")
	}
	want := model.ModelAssignment{ProviderID: "anthropic", ModelID: "claude-opus-4-5"}
	if a != want {
		t.Errorf("kto-orchestrator assignment = %+v, want %+v", a, want)
	}
}
...
func TestReadCurrentModelAssignmentsMalformedModelField(t *testing.T) {
	...
	content := `{
  "agent": {
    "kto-orchestrator": { "model": "no-colon-here" },
	...
	// Malformed kto-orchestrator skipped, sdd-apply parsed
	if _, ok := got["kto-orchestrator"]; ok {
...
func TestReadCurrentModelAssignmentsSlashSeparator(t *testing.T) {
	...
	content := `{
  "agent": {
    "kto-orchestrator": { "model": "zai-coding-plan/glm-5-turbo" }
  }
}`
	if err := os.WriteFile(settingsPath, []byte(content), 0o644); err != nil {
		t.Fatalf("write settings: %v", err)
	...
	a, ok := got["kto-orchestrator"]
	if !ok {
		t.Fatal("kto-orchestrator missing from result — slash-separated format not parsed")
	}
