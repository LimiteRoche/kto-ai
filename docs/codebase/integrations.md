...
| OpenCode plugins | `internal/components/opencodeplugin/` | Register external OpenCode plugin package names; k.to Logo also writes a managed local TUI plugin file and registers its path. |
...
| `internal/components/opencodeplugin/` | External plugin package-name registration; k.to Logo also writes/registers a managed local TUI plugin file. | OpenCode and the plugin package or managed local plugin file. |

## Thin plugin principle

OpenCode community plugins are optional integrations. For external plugins, Gentle-AI ensures `~/.config/opencode/tui.json` exists and contains the plugin package name. For k.to Logo, Gentle-AI writes the managed local TUI plugin file under `~/.config/opencode/tui-plugins/` and registers that path. OpenCode owns runtime loading.
