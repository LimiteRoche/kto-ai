...
**R-PROF-05**: El nombre "default" ESTÁ RESERVADO para el conductor SDD base de OpenCode (`kto-orchestrator`). El usuario NO puede crear un perfil llamado "default".
...
**R-PROF-07b**: El perfil `default` también se PUEDE editar — es el `kto-orchestrator` base. Editar el default es equivalente a lo que hoy hace "Configure Models → OpenCode" pero integrado en el flujo de perfiles.
...
**R-PROF-43**: El perfil default (`kto-orchestrator`) SIEMPRE existe cuando SDD está configurado. Los perfiles adicionales son opcionales.
...
5. **¿El `gentleman` agent (persona) también necesita variantes por perfil?**
   → No. El `gentleman` agent es la persona general, no parte de SDD. El conductor SDD base de OpenCode es `kto-orchestrator`.
